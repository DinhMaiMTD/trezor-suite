...
type Params = PROTO.CKBGetAddress & {
    address?: string;
    coin?: string;
};

const getCkbNetwork = (coin?: string) => {
    if (!coin) return undefined;
    const normalized = coin.toLowerCase();

    return normalized === 'tckb' || normalized === 'testnet' ? 'Testnet' : undefined;
};
...
export default class CkbGetAddress extends AbstractMethod<'ckbGetAddress', Params[]> {
    ...
    init() {
        ...
        this.params = payload.bundle.map(batch => {
            ...
            return {
                ...
                network: getCkbNetwork(batch.coin),
            };
        });
    ...
    get confirmation() {
        return {
            ...
            label:
                this.params.length > 1
                    ? 'Export multiple Nervos CKB addresses'
                    : `Export Nervos CKB address for account #${
